...
function Guess_elimination() {
  ...
  const Guess_eli = (n,MatrixA,MatrixB)=>{
    let A = clone(MatrixA);
    let B = clone(MatrixB);
    let tans = [];
  ...
  const handleChange = (selectedOption) => {
    ...
    Guess_eli(3,selectedOption.dataA,selectedOption.dataB)
  ...
  <div>
    <div style={{textAlign:'center',fontSize:'20px',fontWeight:'bold'}}>Guess-EliminationXDXD</div>
      ...
      <button onClick={()=>Guess_eli(row,valm,valv)}>submit</button>
    </div>
  </div>
  );
}
export default Guess_elimination;